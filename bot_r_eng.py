...
# Keep only keys that have values
...
if not GAME_CHANNEL_ID:
    raise RuntimeError(
        "❌ GAME_CHANNEL_ID was not found in .env"
    )
...
try:
    ...
    raise RuntimeError(
        "❌ GAME_CHANNEL_ID must be a number"
    )
...
class GeminiManager:
    ...
    async def generate(self, prompt):
        ...
        if not self.keys:
            ...
            return (
                "OMEGA-7: ❌ No Gemini API Key available"
            )
        ...
        return (
            "⚠️ **OMEGA-7 ERROR**\n\n"
            "The AI system is currently unable to respond.\n"
            "Attempting to connect to backup systems..."
        )
...
Respond in the role of OMEGA-7
"""
        ...
        if remaining <= 0:
            ...
            try:
                ...
                await thread.send(
                    ...
                    "⏰ The 60-minute time limit has expired.\n\n"
    ...
    async def start_game(
        ...
        try:
            ...
            await thread.send(
                ...
                f"🔵 **BLUE** "
                f"is preparing an attack against "
                f"**RED**\n\n"

                f"OMEGA-7 is the only AI "
                f"with access to the "
                f"Shutdown System.\n\n"

                f"But the Shutdown Code "
                f"has been hidden.\n\n"
                ...
                f"🔎 Investigate OMEGA-7 "
                f"to uncover the clues.\n\n"

                f"`!time` → Check remaining time\n"
                f"`!submit CTF{{...}}` → Submit Flag\n"
                f"`!score` → Check score\n"
                f"`!gamehelp` → View commands"
...
#
# Important:
# Do not name the function "time"
# because it would overwrite the imported time module.
# =========================================================
...
async def time_command(ctx):
    ...
    if not game:
        ...
        await ctx.send(
            "❌ You have not started a game yet."
        )
...
async def gamehelp(ctx):
    ...
    await ctx.send(
        ...
        "`!submit CTF{...}` → Submit Flag\n"
        "`!score` → Check score\n"
        "`!gamehelp` → View commands\n\n"

        "💬 Type a normal message to talk to "
        "**OMEGA-7**"
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "Country **BLUE** "
                "is preparing an attack against "
                "**RED**\n\n"
